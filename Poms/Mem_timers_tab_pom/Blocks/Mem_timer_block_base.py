...
class MemTimerBlockBase:
    def __init__(self, root):
        ...
        self._start_on_sec_input = root.locator(
    ...
    def expect_active_on_create(self, state: bool):
        if state:
            expect(self._active_on_create_checkbox).to_be_checked()
        else:
            expect(self._active_on_create_checkbox).not_to_be_checked()

    def expect_week_day(self, index: int, state: bool):
        checkbox = self._week_day_checkboxes.nth(index)
    ...
    def expect_start_on_sec(self, value: int):
        expect(self._start_on_sec_input).to_have_value(str(value))
